 ...
 */
export async function endElectionService(body) {
  const { electionId } = body;

  if (!electionId) {
    return {
      status: 400,
      body: { error: "Missing required field: electionId" },
    };
  }
  ...
  try {
    ...
    return {
      status: 200,
      body: { message: "Election ended", txHash: tx.hash },
    };
  } catch (err) {
 ...
 * Requires authentication
 */
export async function getAllElectionsService() {
  try {
